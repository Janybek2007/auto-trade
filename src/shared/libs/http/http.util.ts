...
export class HttpClient<T> {
	...
	private async applyRequestMiddlewares(
		params: RequestParams<T>
	): Promise<RequestParams<T>> {
	...
	async request<R>(
		...
		let data = await (requestParams.parser
			? requestParams.parser(response)
			: response.json());
		data = await this.applyResponseMiddlewares(data);
		return data;
	}

	get<R = any>(url: string, config: HTTPRequestConfig = {}): Promise<R> {
		return this.request<R>('GET', url, undefined, config);
	}

	post<R = any>(url: string, body: any, config: HTTPRequestConfig = {}): Promise<R> {
		return this.request<R>('POST', url, body, config);
	}

	put<R = any>(url: string, body: any, config: HTTPRequestConfig = {}): Promise<R> {
		return this.request<R>('PUT', url, body, config);
	}

	delete<R = any>(url: string, config: HTTPRequestConfig = {}): Promise<R> {
