         ...
         <div className={s['f-block']}>
            ...
            {filtrations.map(filtration => (
               <Collapsible
                  ...
                  className={s.block}
                  trigger={<FiltrationTrigger filtration={filtration} toggleSection={toggleSection} openSections={openSections} t={t.get} />}
...
const FiltrationTrigger: React.FC<FiltrationTriggerProps> = React.memo(({ filtration, openSections, toggleSection, t }) => {
   const [focus, setFocus] = useState<string | null>(null);
   const { updateFilter, filters } = useFiltrations();

   const isEditable = ["price", "year_of_production", "mileage"].includes(filtration.value)
    && (focus === filtration.label);

   const limits: Record<string, { min: number; max: number; maxLength: number }> = {
      price: { min: 0, max: 9999999, maxLength: 7 },
      year_of_production: { min: 1900, max: 2025, maxLength: 4 },
      mileage: { min: 0, max: 9999999, maxLength: 7 },
   };
   ...
   return (
      <div className={`${s.trigger}`} onClick={() => { if (!focus) toggleSection(filtration.value); }}>
